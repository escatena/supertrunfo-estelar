...
constructor(p1,p2, room) {
    ...
    [1.92,1.84,8590,24.8,16.6,1.15,"Fomalhaut","Peixes"],
    ...
    player.on('pass_turn', (opt,turn, room)=> {
      if (room === this.sala) {
...
qualTurno(item,p,index) {
    ...
    this.escolha = item;
      ...
      if (c1[this.escolha] > c2[this.escolha]) {
        ...
        this.players[0].emit("resultado", c1, c2,this.score1,this.score2, this.escolha,"lime", "red", this.turno, this.sala);
        this.players[1].emit("resultado", c2, c1,this.score2,this.score1, this.escolha,"red", "lime", this.turno, this.sala);
        }
       else if (c2[this.escolha] > c1[this.escolha]) {
        this.turno = 1;
        this.score2=this.score2+1;
        this.players[1].emit("resultado", c2, c1, this.score2,this.score1,this.escolha,"lime","red", this.turno, this.sala) ;
        this.players[0].emit("resultado",c1, c2, this.score1,this.score2,this.escolha,"red","lime", this.turno, this.sala);
      } 
      else {
        this.players[0].emit("resultado",c1, c2, this.score1,this.score2,this.escolha,"yellow","yellow", this.turno, this.sala);
        this.players[1].emit("resultado",c2, c1, this.score2,this.score1,this.escolha,"yellow","yellow",this.turno, this.sala);
      }

      if (this.rodada < this.cartas[0].length-1) {
...
final() {
  ...
  if (final > 0) {
    this.players[0].emit("final", this.score1, this.score2, "Parabéns, você ganhou!", this.sala);
    this.players[1].emit("final", this.score2, this.score1, "Você perdeu dessa vez!", this.sala);
   this.reset();
  }
  else if (final<0){
    this.players[1].emit("final", this.score2, this.score1, "Parabéns, você ganhou!", this.sala);
    this.players[0].emit("final", this.score1, this.score2, "Você perdeu dessa vez!", this.sala);
    this.reset();
  }
  else {
    this.players[0].emit("final", this.score1, this.score2, "Houve um empate!", this.sala);
    this.players[1].emit("final", this.score2, this.score1, "Houve um empate!", this.sala);
    this.reset();
  }
